--
-- Type: PROCEDURE; Owner: TM_CZ; Name: I2B2_PROCESS_MRNA_INC_DATA_Old
--
  CREATE OR REPLACE PROCEDURE "TM_CZ"."I2B2_PROCESS_MRNA_INC_DATA_Old" 
(
  trial_id 		VARCHAR2
 ,top_node		varchar2
  ...
  END IF;
	...
	where trial_name = TrialID 
	  ...
	  and platform = 'MRNA_AFFYMETRIX'
          and (subject_id||category_cd) in (select (subject_id||category_cd) from lt_src_mrna_subj_samp_map); --Making sure only mRNA data is deleted
...
--		  ,sd.subject_id
	...
	where sd.sample_cd = md.expr_id
	  ...
	  and decode(dataType,'R',sign(md.intensity_value),1) = 1  --	take only >0 for dataType R
           and (sd.subject_id||sd.category_cd) in (select (subject_id||category_cd) from lt_src_mrna_subj_samp_map) 
