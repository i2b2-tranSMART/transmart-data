...
BEGIN
  ...
  stepCt := 0;
  stepCt := stepCt + 1;
  cz_write_audit(jobId,databaseName,procedureName,'Starting i2b2_process_acgh_data',0,stepCt,'Done');
 ...
 -- check if platform exists in de_chromosomal_region .  If not, abort run.
  ...
  select count(*) into pExists
    from all_tables
      where table_name  = 'DE_SUBJECT_ACGH_DATA' and
	    partitioned = 'YES';

  if pExists = 0 then
	--	dataset is not partitioned so must delete
	delete from de_subject_acgh_data
	where trial_source = TrialId || ':' || sourceCd;
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Delete data from de_subject_acgh_data',SQL%ROWCOUNT,stepCt,'Done');
	commit;
  else
	--	Create partition in de_subject_acgh_data if it doesn't exist else truncate partition
	select count(*)	into pExists
	...
	if pExists = 0 then
		--	needed to add partition to de_subject_acgh_data
		sqlText := 'alter table deapp.de_subject_acgh_data add PARTITION "' ||
			   TrialID                                                        ||
                           '"  VALUES ('                                                  ||
                           ''''                                                           ||
                           TrialID                                                        ||
		...
		stepCt := stepCt + 1;
		cz_write_audit(jobId,databaseName,procedureName,'Adding partition to de_subject_acgh_data',0,stepCt,'Done');
	else
		sqlText := 'alter table deapp.de_subject_acgh_data truncate partition "' || TrialID || '"';
		execute immediate(sqlText);
		stepCt := stepCt + 1;
		cz_write_audit(jobId,databaseName,procedureName,'Truncating partition in de_subject_acgh_data',0,stepCt,'Done');
	end if;
  ...
  delete from DE_SUBJECT_SAMPLE_MAPPING
    where trial_name           = TrialID     and
	  ...
	  platform             = 'ACGH'; --Making sure only mRNA data is deleted
  ...
  insert into deapp.de_subject_acgh_data
	(   region_id
          , assay_id
          , patient_id
          , trial_name
  ...
  stepCt := stepCt + 1;
  cz_write_audit(jobId,databaseName,procedureName,'Insert into DEAPP wt_subject_acgh_region',SQL%ROWCOUNT,stepCt,'Done');
  commit;
  ...
  stepCt := stepCt + 1;
  cz_write_audit(jobId,databaseName,procedureName,'End i2b2_process_acgh_data',0,stepCt,'Done');
