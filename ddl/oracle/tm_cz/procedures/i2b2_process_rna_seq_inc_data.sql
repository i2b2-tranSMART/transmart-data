--
-- Type: PROCEDURE; Owner: TM_CZ; Name: I2B2_PROCESS_RNA_SEQ_INC_DATA
--
  CREATE OR REPLACE PROCEDURE "TM_CZ"."I2B2_PROCESS_RNA_SEQ_INC_DATA"
(
  trial_id 		VARCHAR2
 ,top_node		varchar2
		 ...
		 (select distinct (x.sample_cd||x.patient_id)
		  ...
		  where x.trial_name = TrialId
		    ...
		    and x.platform = 'RNA_SEQ'
                   and (x.subject_id||x.sample_cd) in (select (subject_id||sample_cd) from lt_src_RNA_SEQ_subj_samp_map));
...
--	Cleanup any existing data in de_subject_sample_mapping.
	...
	where trial_name = TrialID
	  ...
	  and (subject_id||sample_cd) in (select (subject_id||sample_cd) from lt_src_RNA_SEQ_subj_samp_map); --Making sure only RNA_sequencing data is deleted
	...
	commit;
       ...
       delete from de_subject_rna_data
		...
		--and patient_id in (select p.patient_num from patient_dimension p,lt_src_RNA_SEQ_subj_samp_map s where
...
--	Insert records for patients and samples into observation_fact
    ...
    where m.trial_name = TrialID
      ...
      and m.platform = 'RNA_SEQ'
       and (m.subject_id||m.sample_cd) in (select (subject_id||sample_cd) from lt_src_RNA_SEQ_subj_samp_map);
...
--		  ,sd.subject_id
	...
	--	wt_subject_RNA_seq_probeset as part of a Load.
		...
		if dataType = 'R' or dataType = 'L' then
			I2B2_RNASEQ_INC_ZSCORE_CALC(TrialID,'L',jobId,dataType,logBase,sourceCD);
			stepCt := stepCt + 1;
