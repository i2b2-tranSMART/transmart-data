--
-- Type: PROCEDURE; Owner: TM_CZ; Name: I2B2_PROC_METABOLOMIC_INC_DATA
--
  CREATE OR REPLACE PROCEDURE "TM_CZ"."I2B2_PROC_METABOLOMIC_INC_DATA" 
(
  trial_id 		VARCHAR2
 ,top_node		varchar2
  ...
  END IF;
	...
	*/
		 ...
		 (select distinct (x.sample_cd||x.patient_id)
		  ...
		  where x.trial_name = TrialId
		    ...
		    and x.platform = 'METABOLOMICS'
                     and (x.subject_id||x.sample_cd) in (select (subject_id||sample_cd) from LT_SRC_METABOLOMIC_MAP));
	...
	where trial_name = TrialID 
	  ...
	  and (subject_id||sample_cd) in (select (subject_id||sample_cd) from LT_SRC_METABOLOMIC_MAP); --Making sure only metabolomic data is deleted
	...
	delete from DE_SUBJECT_METABOLOMICS_DATA
		...
		--and patient_id in (select p.patient_num from patient_dimension p,LT_SRC_METABOLOMIC_MAP s 
		--where p.sourcesystem_cd=regexp_replace(TrialID || ':' || s.site_id || ':' || s.subject_id,'(::){1,}', ':'))
		;
		stepCt := stepCt + 1;
		cz_write_audit(jobId,databaseName,procedureName,'Delete data from DE_SUBJECT_METABOLOMICS_DATA for  updation',SQL%ROWCOUNT,stepCt,'Done');
		commit;
...
--	Insert records for patients and samples into observation_fact
    ...
    where m.trial_name = TrialID 
      ...
      and m.platform = 'METABOLOMICS'
        and (m.subject_id||m.sample_cd) in (select (subject_id||sample_cd) from LT_SRC_METABOLOMIC_MAP);
...
--	,sample_cd
	...
	--  and md.peptide =p.peptide-- gs.mirna_id
	 ...
	 and (sd.subject_id||sd.sample_cd) in (select (subject_id||sample_cd) from LT_SRC_METABOLOMIC_MAP) 
	...
	--	WT_SUBJECT_MBOLOMICS_PROBESET as part of a Load.  
		...
		if dataType = 'R' or dataType = 'L' then
			I2B2_METBOLO_INC_ZSCORE_CALC(TrialID,'L',jobId,dataType,logBase,sourceCD);
			stepCt := stepCt + 1;
