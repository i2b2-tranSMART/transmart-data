--
-- Type: PROCEDURE; Owner: TM_CZ; Name: I2B2_PROCESS_MIRNA_INC_DATA
--
  CREATE OR REPLACE PROCEDURE "TM_CZ"."I2B2_PROCESS_MIRNA_INC_DATA" 
(
  trial_id 		VARCHAR2
 ,top_node		varchar2
  ...
  END IF;
		 ...
		 (select distinct (x.sample_cd||x.patient_id)
		  ...
		  where x.trial_name = TrialId
		    ...
		    and x.platform = mirna_type
                     --and (x.subject_id||x.category_cd) in (select (subject_id||category_cd) from LT_SRC_MIRNA_SUBJ_SAMP_MAP));
                     and (x.subject_id||x.sample_cd) in (select (subject_id||sample_cd) from LT_SRC_MIRNA_SUBJ_SAMP_MAP)); ---UAT 163 changes made on 25/02/2014
	...
	where trial_name = TrialID 
	  ...
	  and platform = mirna_type
	   --and (subject_id||category_cd) in (select (subject_id||category_cd) from LT_SRC_MIRNA_SUBJ_SAMP_MAP); --Making sure only miRNA data is deleted
          and (subject_id||sample_cd) in (select (subject_id||sample_cd) from LT_SRC_MIRNA_SUBJ_SAMP_MAP); --UAT 163 changes made on 25/02/2014
	stepCt := stepCt + 1;
	...
	commit;
		...
		--and patient_id in (select p.patient_num from patient_dimension p,LT_SRC_MIRNA_SUBJ_SAMP_MAP s 
               -- where p.sourcesystem_cd=regexp_replace(TrialID || ':' || s.site_id || ':' || s.subject_id,'(::){1,}', ':'))
                ;
		stepCt := stepCt + 1;
		cz_write_audit(jobId,databaseName,procedureName,'Delete data from de_subject_mirna_data for  updation',SQL%ROWCOUNT,stepCt,'Done');
		commit;
	...
	if pExists = 0 then
		...
		if pExists = 0 then
			...
			sqlText := 'alter table deapp.de_subject_mirna_data add PARTITION "' || TrialID || '"  VALUES (' || '''' || TrialID || '''' || ') ' ||
...
--	Insert records for patients and samples into observation_fact
    ...
    where m.trial_name = TrialID 
      ...
      and m.platform = mirna_type
       and (m.subject_id||m.sample_cd) in (select (subject_id||sample_cd) from LT_SRC_MIRNA_SUBJ_SAMP_MAP);
   ...
   for ul in uploadI2b2
	...
	SET n.c_columndatatype = 'T',
      ...
      from tm_lz.lt_src_mirna_display_mapping m where m.category_cd=ul.category_cd)||
                '</ValueMetadata>') where n.c_fullname=(select leaf_node from WT_QPCR_MIRNA_NODES where category_cd=ul.category_cd and leaf_node=n.c_fullname and leaf_node=n.c_fullname );
...
--	,subject_id
	...
	/*update de_subject_mirna_data d set (d.probeset_id            
						,d.assay_id              
						,d.zscore )=
						(select  p.probeset_id 			
							 ,p.assay_id			
							 ,(case when p.intensity_value < -2.5
								...
								then 2.5
								 ...
								 from wt_subject_mirna_probeset p
                                                                 ,de_subject_mirna_data da
 ...
 stepCt := stepCt + 1;
 cz_write_audit(jobId,databaseName,procedureName,'Update into DEAPP de_subject_microarray_data',SQL%ROWCOUNT,stepCt,'Done');*/
	...
	--	wt_subject_mirna_probeset as part of a Load.  
		...
		if dataType = 'R' or dataType = 'L' then
			...
			i2b2_mirna_inc_zscore_calc(TrialID,'L',jobId,'R',logBase,sourceCD);----donot do log transform
                        else
                        i2b2_mirna_inc_zscore_calc(TrialID,'L',jobId,'L',logBase,sourceCD);----do log transform
                        end if;
			stepCt := stepCt + 1;
