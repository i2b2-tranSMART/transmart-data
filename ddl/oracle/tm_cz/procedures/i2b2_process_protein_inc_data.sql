--
-- Type: PROCEDURE; Owner: TM_CZ; Name: I2B2_PROCESS_PROTEIN_INC_DATA
--
  CREATE OR REPLACE PROCEDURE "TM_CZ"."I2B2_PROCESS_PROTEIN_INC_DATA" 
(
  trial_id 		VARCHAR2
 ,top_node		varchar2
...
BEGIN
  ...
  END IF;
	...
	stepCt := 0;
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Starting i2b2_process_proteomics_data',0,stepCt,'Done');
	...
	*/
	...
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Uppercase trial_name in LT_SRC_PROTEOMICS_SUB_SAM_MAP',SQL%ROWCOUNT,stepCt,'Done');
	commit;	
		 ...
		 (select distinct (x.sample_cd||x.patient_id)
		  ...
		  where x.trial_name = TrialId
		    ...
		    and x.platform = 'PROTEIN'
                     and (x.subject_id||x.sample_cd) in (select (subject_id||sample_cd) from LT_SRC_PROTEOMICS_SUB_SAM_MAP));
	...
	and (subject_id||sample_cd) in (select (subject_id||sample_cd) from LT_SRC_PROTEOMICS_SUB_SAM_MAP); --Making sure only miRNA data is deleted
	...
	commit;
		...
		--and patient_id in (select p.patient_num from patient_dimension p,LT_SRC_PROTEOMICS_SUB_SAM_MAP s where 
               -- p.sourcesystem_cd=regexp_replace(TrialID || ':' || s.site_id || ':' || s.subject_id,'(::){1,}', ':'))
                ;
		stepCt := stepCt + 1;
		cz_write_audit(jobId,databaseName,procedureName,'Delete data from DE_SUBJECT_PROTEIN_DATA for  updation',SQL%ROWCOUNT,stepCt,'Done');
		commit;
...
--	from wt_subject_mirna_data
	...
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Insert node values into DEAPP WT_PROTEOMICS_NODE_VALUES',SQL%ROWCOUNT,stepCt,'Done');
	commit;
    ...
    stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Create leaf nodes in DEAPP tmp_proteomics_nodes',SQL%ROWCOUNT,stepCt,'Done');
	commit;
    ...
    stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Create platform nodes in WT_PROTEOMICS_NODES',SQL%ROWCOUNT,stepCt,'Done');
	commit;
    ...
    stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Create ATTR1 nodes in WT_PROTEOMICS_NODES',SQL%ROWCOUNT,stepCt,'Done');
	commit;
    ...
    stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Create ATTR2 nodes in WT_PROTEOMICS_NODES',SQL%ROWCOUNT,stepCt,'Done');
	commit;
    ...
    stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Updated node_name in DEAPP tmp_proteomics_nodes',SQL%ROWCOUNT,stepCt,'Done');
	commit;
...
--	update concept_cd for nodes, this is done to make the next insert easier
	...
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Update WT_PROTEOMICS_NODES with newly created concept_cds',SQL%ROWCOUNT,stepCt,'Done');
	commit;
  ...
  --ASSAY_ID        = generated by trigger
	...
	from (select distinct b.patient_num as patient_id
			  ...
			  ,a.sample_cd
			  ,nvl(a.category_cd,'Biomarker_Data+PROTEOMICS+PLATFORM+TISSUETYPE+ATTR1+ATTR2') as category_cd
			  ,a.source_cd
...
--	Insert records for patients and samples into observation_fact
    ...
    where m.trial_name = TrialID 
      ...
      and (m.subject_id||m.sample_cd) in (select (subject_id||sample_cd) from LT_SRC_PROTEOMICS_SUB_SAM_MAP);
...
--	,sample_cd
	...
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Insert into DEAPP WT_SUBJECT_PROTEOMICS_PROBESET',SQL%ROWCOUNT,stepCt,'Done');
...
*/	
	...
	--	WT_SUBJECT_PROTEOMICS_PROBESET as part of a Load.  
		...
		if dataType = 'R' or dataType = 'L' then
			I2B2_PROTEIN_INC_ZSCORE_CALC(TrialID,'L',jobId,dataType,logBase,sourceCD);
			stepCt := stepCt + 1;
	...
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'End i2b2_process_proteomics_data',0,stepCt,'Done');
	...
	when unmapped_platform then
		cz_write_audit(jobId,databasename,procedurename,'Platform not found in de_qpcr_proteomics_annotation',1,stepCt,'ERROR');
		CZ_ERROR_HANDLER(JOBID,PROCEDURENAME);
		cz_end_audit (jobId,'FAIL');
		select 163 into rtn_code from dual;--mod
	when multiple_platform then
		cz_write_audit(jobId,databasename,procedurename,'Multiple platforms for sample_cd in LT_SRC_PROTEOMICS_SUB_SAM_MAP',1,stepCt,'ERROR');
		CZ_ERROR_HANDLER(JOBID,PROCEDURENAME);
		cz_end_audit (jobId,'FAIL');
